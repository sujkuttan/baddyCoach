...
interface ReportViewProps {
  ...
  onLabeling?: (report: any) => void;
  onRedoCourt?: (jobId: string) => void;
}
...
export function ReportView({ jobId, reportData, onBack, onViewProgress, onLabeling, onRedoCourt }: ReportViewProps) {
      ...
      <header className="sticky top-0 z-50 bg-court-dark/90 backdrop-blur-md border-b border-court-line/15">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            {onLabeling && report && (
              <button
                onClick={() => onLabeling(report)}
                className="flex items-center gap-2 px-3 py-1.5 rounded-lg border border-warning-yellow/30 text-warning-yellow hover:bg-warning-yellow/10 transition-colors"
              >
                <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" />
                </svg>
                <span className="font-mono text-[10px] tracking-wider">LABEL SHOTS</span>
              </button>
            )}
            {jobId && onRedoCourt && (
              <button
                onClick={() => onRedoCourt(jobId)}
                className="flex items-center gap-2 px-3 py-1.5 rounded-lg border border-court-line/30 text-text-muted hover:text-shuttle-lime hover:border-shuttle-lime/30 transition-colors"
              >
                <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                </svg>
                <span className="font-mono text-[10px] tracking-wider">COURT SETUP</span>
              </button>
            )}
